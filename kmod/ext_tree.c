...
struct ext_tree *ext_tree_create(struct nkfs_sb *sb, u64 block)
{
	...
	tree->sb = sb;
	tree->btree = nkfs_btree_create(sb, block);
	if (!tree->btree)
		goto fail;
...
void ext_to_nkfs_btree_key_value(struct ext *ext,
		struct nkfs_btree_key *key, struct nkfs_btree_value *value)
{
	nkfs_btree_key_by_u64(ext->start, key);
	nkfs_btree_value_by_u64(ext->size, value);
}

void ext_by_nkfs_btree_key_value(struct nkfs_btree_key *key,
		struct nkfs_btree_value *value, struct ext *ext)
{
	ext->start = nkfs_btree_key_to_u64(key);
	ext->size = nkfs_btree_value_to_u64(value);
}

int ext_find_fit_clb(void *ctx, struct nkfs_btree_node *node, int index)
{
	...
	key = nkfs_btree_node_key(node, index);
	value = nkfs_btree_node_value(node, index);
	ext_by_nkfs_btree_key_value(key, value, &ext);
...
restart:
	...
	need.size = size;
	nkfs_btree_read_lock(tree->btree);
	if (!nkfs_btree_enum_tree(tree->btree, ext_find_fit_clb, &need)) {
		err = -ENOENT;
		nkfs_btree_read_unlock(tree->btree);
		goto out;
	}
	nkfs_btree_read_unlock(tree->btree);
	nkfs_btree_write_lock(tree->btree);
	if (!need.node->block || (need.index >= need.node->nr_keys)) {
		nkfs_btree_write_unlock(tree->btree);
		NKFS_BTREE_NODE_DEREF(need.node);
		goto restart;
	}

	pkey = nkfs_btree_node_key(need.node, need.index);
	pvalue = nkfs_btree_node_value(need.node, need.index);
	ext_by_nkfs_btree_key_value(pkey, pvalue, &ext);
	if (ext.size < size) {
		nkfs_btree_write_unlock(tree->btree);
		NKFS_BTREE_NODE_DEREF(need.node);
		goto restart;
	...
	if ((ext.size - size) == 0) {
		err = nkfs_btree_node_delete_key(tree->btree->root, pkey);
		if (err)
			goto out;
		*pext = ext;
	} else {
		nkfs_btree_value_by_u64(ext.size - size, pvalue);
		err = nkfs_btree_node_write(need.node);
		if (err)
			goto out;
